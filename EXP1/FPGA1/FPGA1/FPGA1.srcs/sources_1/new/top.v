...
module top #(parameter n=`N, m=`M)
   ...
   // output rx_ready,
    ...
    localparam COUNTER_SIZE = 31,
        SAMPLES_TO_COLLECT = 1200,  // per TDC
        NUM_AES_CIRCUITS = 4;
    ...
    // TDC FPGA Design
    wire clk_temp, clk_temp1, clk_temp_bufg;
   
   clk_filt_PLL cf0( .clk_in(clk0), .clk_out(clk_TDC));
   ...
   // uart_rx uartRX_TDC(.i_Clock(clk1), .i_Rx_Serial(rx_TDC), .o_Rx_DV(rxReady_TDC), .o_Rx_Byte(RXdata_TDC) );	
