...
module top_TDC_ZED_BOARD 
    ...
    wire locked;
    wire clkref;
   // clock clock(clk, clk_cou_in, clk1, clk2, clk3, clk4, clk5t, locked);
    ...
    uart_tx uartTX(.i_Clock(clk), .i_Tx_DV(transmitReg), .i_Tx_Byte(TXdata), .o_Tx_Active(txActive), .o_Tx_Serial(tx), .o_Tx_Done(TXDone) );		
    uart_rx uartRX(.i_Clock(clk), .i_Rx_Serial(rx_TDC), .o_Rx_DV(rxReady_TDC), .o_Rx_Byte(RXdata) );		
    ...
    // TDC1
    tdc_top tp (clk, clk, out);					// TDC sensor
    tdc_decode tdc_decode(.clk(clk), .rst(rst), .chainvalue_i(outReg), .coded_o(processedOut));   // calculate number of 1's in the TDC Sensor
    

// 2nd tdc
    tdc_top tp1 (clk, clk_out0, out1); // TDC sensor
    tdc_decode tdc1_decode(.clk(clk), .rst(rst), .chainvalue_i(outReg1), .coded_o(processedOut1));   // calculate number of 1's in the TDC Sensor
